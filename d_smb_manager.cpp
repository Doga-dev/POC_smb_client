                              ...
                              const QByteArray & content)
{
    ...
    if (context) {
        ...
        if (contextInfo) {
            ...
            if (fileHandle >= 0) {
                ssize_t bytesWriten = smbc_write(fileHandle, content.toStdString().c_str(), content.size());
                                         ...
                                         const QString & sourcePath)
{
    ...
    if (context) {
        ...
        if (contextInfo) {
            ...
            QString destBaseUrl = QString("smb://%1/%2/%3").arg(contextInfo->address,
                                                             contextInfo->sharedFolder,
                                                             relativeDestPath);
            smbc_set_context(context);

            QDirIterator it(sourcePath, QDir::Files | QDir::Dirs | QDir::NoDotAndDotDot | QDir::NoSymLinks, QDirIterator::Subdirectories);
            int filesCopied = 0;

            while (it.hasNext()) {
                it.next();
                QString localPath = it.filePath();
                QString relativePath = it.filePath().mid(sourcePath.length() + 1); // Chemin relatif par rapport au sourcePath
                ...
                if (it.fileInfo().isDir()) {
                    ...
                    if (smbc_mkdir(destPath.toStdString().c_str(), 0777) != 0) {
                        if (errno != EEXIST) { // Si le répertoire n'existe pas, le créer
                            errMsg = "Erreur : Impossible de créer le répertoire distant : " + destPath;
                            break;
                        }
                        qDebug() << Q_FUNC_INFO << ": context =" << contextId << ", make directory:" << destPath;
                    }
                    ...
                    if (errMsg.isEmpty()) {
                        ++filesCopied;
                        qDebug() << Q_FUNC_INFO << ": context =" << contextId << ", files copied =" << QString("%1").arg(filesCopied, 6)
                                 << ", sourcePath =" << localPath << ", destPath =" << destPath;
                    } else {
                        break;
                    }
                } else {
                    qDebug() << Q_FUNC_INFO << ": context =" << contextId << ",   unmanaged item:" << localPath;
                }
            }
            if (errMsg.isEmpty()) {
